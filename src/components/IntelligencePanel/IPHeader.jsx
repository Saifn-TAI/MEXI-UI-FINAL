...
const FUNC_LABELS = {
  func_production: 'Production', func_supply: 'Supply', func_vendor: 'Vendor',
  func_demand: 'Demand', func_inventory: 'Inventory', func_quality: 'Quality', func_maintenance: 'Maintenance',
};

export default function IPHeader({ sig, currentPanelSig, badgeCls, badgeLabel, formatId, togglePanelMode, closePanel, activeTab, setActiveTab, headerFlash, originBizCard, onBackToBiz, originFuncCard, onBackToFunc, originAllSignals, onBackToAllSignals }) {
  const originCard = BIZ_IMPACT_CARDS.find(c => c.id === originBizCard);
  const funcLabel = originFuncCard ? FUNC_LABELS[originFuncCard] : null;

  const backLabel = originAllSignals ? 'All Signals' : originCard ? originCard.label : funcLabel;
  const onBack = originAllSignals ? onBackToAllSignals : originCard ? () => onBackToBiz(originCard.id) : funcLabel ? () => onBackToFunc(originFuncCard) : null;
    ...
    <div className={`ip-hdr ${headerFlash ? 'ip-hdr-flash' : ''}`}>
      <div className="ip-hdr-top">
        <div className="ip-hdr-left">
          {onBack && (
            <div className="ip-breadcrumb-icon" onClick={onBack} title={`Back to ${backLabel}`}>
