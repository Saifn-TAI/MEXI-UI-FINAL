    ...
    <>
      <div className="stack-hdr">
        ...
        <div className="stack-hdr-right">
          ...
          <div className="stack-view-all" onClick={openAllSignalsPanel}>
            <span>{allSigs.length} active signals →</span>
          </div>
        </div>
      </div>
      <div className="signal-stack">
        {topCritical.map((id, index) => {
          ...
          return (
            <div key={id} className={`psc sev-${s.sev}`} data-sig={id}>
              <div className="psc-top-bar"></div>
              <div className="psc-body">
                <div className="psc-row1">
                  <div className="psc-rank-badge">
                    <span className="psc-rank-num">#{rank}</span>
                    <span className={`psc-sev ${s.sev}`}><span className="psc-sev-dot"></span>{s.sev === 'r' ? 'Critical' : s.sev === 'a' ? 'Warning' : 'Normal'}</span>
                  </div>
